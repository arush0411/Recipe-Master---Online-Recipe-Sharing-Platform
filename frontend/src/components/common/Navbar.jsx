  ...
  return (
    <Box w="min(100%,80rem)" mx="auto">
      <Flex
        ...
        borderColor={borderColor}  // subtle border
      >
        ...
        <Flex flex={{ base: 1 }} justify={{ lg: "space-between", md: "space-between", base: "flex-end" }}>
          <Text
            as={Link}
            to="/"
            fontSize="2xl"
      ...
      <Collapse in={isOpen} animateOpacity>
        <MobileNav />
        <Stack flex={{ base: 1, md: 0 }} justify={"flex-start"} direction={"row"} spacing={{ base: 2, md: 4 }} p={4}>
          {isAuth ? (
            <>
              <Popover trigger={"hover"} placement={"top-end"}>
                <PopoverTrigger>
                  <IconButton
                    variant="link"
                    color={textColor}
                    _hover={{ opacity: 1, color: "primary.500" }}
              ...
              <Button onClick={logoutHandler} size={{ lg: "lg", md: "md", base: "sm" }}>
                Logout
              </Button>
            </>
          ) : (
            <>
              <Button as={Link} to="/login" variant={"outline"} size={{ lg: "lg", md: "md", base: "sm" }}>
                Login
              </Button>
              <Button as={Link} to="/signup" size={{ lg: "lg", md: "md", base: "sm" }}>
                SignUp
              </Button>
            </>
          )}
...
const DesktopNav = () => {
  ...
  return (
    <Flex gap="1rem" alignItems={"center"}>
      <Flex gap="1rem" alignItems={"center"}>
        <Text as={Link} to="/explore" color={linkColor} _hover={{ textDecoration: "none", color: linkHoverColor }}>
          Explore
        </Text>
        <Text as={Link} to="/feed" color={linkColor} _hover={{ textDecoration: "none", color: linkHoverColor }}>
          Feed
        </Text>
        <Text as={Link} to="/account" color={linkColor} _hover={{ textDecoration: "none", color: linkHoverColor }}>
          Account
        </Text>
      </Flex>

      <Stack flex={{ base: 1, md: 0 }} justify={"flex-end"} direction={"row"} spacing={{ base: 2, md: 4 }}>
        {isAuth ? (
          <>
            <Popover trigger={"hover"} placement={"top-end"}>
              <PopoverTrigger>
